...
export const getOrderDetailById = (id: number): SellerOrderDetail | null => {
  ...
  const orderData: Record<
    number,
    { orderDate: string; deliveryDate: string; status: string; amount: number }
  > = {
    1: {
      orderDate: "01/15/2025",
      deliveryDate: "01/16/2025",
  ...
  const statusMap: Record<
    string,
    "Out For Delivery" | "Received" | "Payment Pending" | "Cancelled"
  > = {
  ...
  const generateItems = (amount: number): OrderDetailItem[] => {
    ...
    while (remainingAmount > 0 && srNo <= 5) {
      ...
      if (itemTotal <= remainingAmount) {
        ...
        const lastQty = Math.max(
          1,
          Math.floor(remainingAmount / (product.basePrice * 1.18))
        );
